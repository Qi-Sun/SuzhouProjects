...
namespace DianPingMarkerMaker
{
    public class DatabaseAccess
    {
        ...
        public DatabaseAccess(string cityName, string pwd)
        {
            DBpwd = pwd;
        }     

        public static string GetConnectionString(string Schema, string pwd)
        {
            //return "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=E:\\My Course\\数据库\\Student.mdb";
            return "Data Source=127.0.0.1;Database=" + Schema + ";User ID=root;Password=" + pwd;
        ...
        public static string GetConnectionString(string ip,string schema,string user,string pwd)
        {
            return string.Format("Data Source={0};Database={1};User ID={2};Password={3}", ip, schema, user, pwd);
        }

        //没用到过
        private static MySqlDataReader DataReader_ExecuteReader(string connectionStringStr, string sqlStr)
        {
