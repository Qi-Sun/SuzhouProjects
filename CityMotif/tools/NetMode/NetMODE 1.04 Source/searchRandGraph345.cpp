...
void* searchRandGraph345(void *darg)
{
	...
	for (root = 1; root <= graph_size; ++ root){
		...
		do{
			if (isNewCall){
				++ depth;
				if (remain == 0){
					++ arg->this_subgraph_count[ arg->pattern[x] ];
					isNewCall = 0;
				} else{ // remain != 0
	...
	if(verbose) { 
		...
		for (int i = 1; i <= arg->pattern_num; ++ i){
			arg->subgraph_concentration[i]   += arg->this_subgraph_count[i] / count_sum;
			arg->subgraph_concentration_2[i] += SQR(arg->this_subgraph_count[i] / count_sum);
		}
		for (int i = 1; i <= arg->pattern_num; ++ i){
